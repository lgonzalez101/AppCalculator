...
locations_normal = {
	...
	'crosswalk': 'development/crosswalk/assets/src',
...
locations_debug = {
	...
	'crosswalk': 'crosswalk/ForgeInspector/assets/src',
	'osx': 'development/osx/Forge.app/Contents/Resources/assets/src',
	'chrome': 'development/chrome/src',
	'firefox': 'development/firefox/resources/f/data/src',
	'safari': 'development/forge.safariextension/src',
...
def copy_user_source_to_template(ignore_patterns=None, src='src', debug=False):
	...
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'copy_files': { 'from': src, 'to': locations["crosswalk"], 'ignore_patterns': ignore_patterns }}},
...
def include_platform_in_html(debug=False):
	...
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'find_and_replace_in_dir': {
			"root_dir": locations["crosswalk"],
			"find": "<head>",
			"replace": "<head><script src='%{back_to_parent}%forge/app_config.js'></script><script src='%{back_to_parent}%forge/all.js'></script>"
		}}},
...
def include_reload():
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'generate_sha1_manifest': {
			"input_folder": "development/crosswalk/assets/src",
			"output_file": "development/crosswalk/assets/hash_to_file.json"
		}}},
...
def include_config(debug=False):
	if debug:
		return [
			...
			{'when': {'platform_is': 'crosswalk'}, 'do': {'write_config': {
				"filename": 'android/ForgeInspector/assets/app_config.json',
				"content": "${config}"
			}}},
			{'when': {'platform_is': 'crosswalk'}, 'do': {'write_config': {
		...
		return [
			...
			{'when': {'platform_is': 'crosswalk'}, 'do': {'write_config': {
				"filename": 'development/crosswalk/assets/app_config.json',
				"content": "${config}"
			}}},
			{'when': {'platform_is': 'crosswalk'}, 'do': {'write_config': {
				"filename": 'development/crosswalk/assets/forge/app_config.js',
				"mapping_file": 'development/crosswalk/assets/module_mapping.json',
				"content": "window.forge = {}; window.forge.config = ${config}; window.forge.module_mapping = ${module_mapping};"
			}}},
...
def run_module_build_steps(build):
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {
			'run_module_build_steps': {
				'steps_path': 'development/crosswalk/build_steps',
				'src_path': 'development/crosswalk/assets/src',
				'project_path': 'development/crosswalk'
			}
		}},
...
def run_android_phase(build_type_dir, sdk, device, interactive, purge=False):
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'run_android': (build_type_dir, sdk, device, interactive, purge)}},
	]
...
def package(build_type_dir):
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'package_android': ()}},
...
def serve(build_type_dir):
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'serve_android': ()}},
...
def clean_phase():
	return [
		...
		{'when': {'platform_is': 'crosswalk'}, 'do': {'clean_android': ()}},

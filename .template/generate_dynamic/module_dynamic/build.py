...
def apply_module_to_android_project(module_path, project_path, app_config, skip_jar=False, include_tests=False, local_build_steps=None):
	...
	crosswalk_aar = os.path.join(module_path, 'android', 'ForgeModule-crosswalk-release.aar')
	...
	if not skip_jar:
		...
		if os.path.exists(crosswalk_aar) and os.path.exists(vanilla_aar):
			LOG.info("Android module '%s': Adding module aar to libs" % module_name)
			shutil.copy2(crosswalk_aar, os.path.join(project_path, 'libs', module_name+'-crosswalk-release.aar'))
	...
	if os.path.isdir(module_libs) and (not (os.path.exists(crosswalk_aar) and os.path.exists(vanilla_aar))):

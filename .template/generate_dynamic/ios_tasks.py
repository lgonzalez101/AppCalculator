...
See "Preparing your apps for app stores" in our docs: [https://trigger.io/docs/current/recipes/release/release_mobile.html]'''.format(
	...
	def create_ipa_from_app(self, build, provisioning_profile, output_path_for_ipa, certificate_to_sign_with=None, relative_path_to_itunes_artwork=None, certificate_path=None, certificate_password=None, output_path_for_manifest=None):
		...
		with lib.temp_dir() as temp_dir:
			...
			LOG.info("COPYING TREE: %s -> %s" % (path_to_app, path.join(path_to_payload, path.basename(path_to_app))))
	...
	def _locate_device_app(self, build, error_message):
		...
		LOG.info("Locating ios app for target: %s -> %s" % (target, self.path_to_ios_build))
	...
	def run_iphone_simulator(self, build):
		...
		if path.exists(SIMULATOR_IN_70) or path.exists(SIMULATOR_IN_70_beta):
			LOG.debug("Detected XCode version 7 or newer")
			ios_sim_binary = "ios_sim.py"
		elif path.exists(SIMULATOR_IN_60):
		...
		# On Xcode 7 or newer we are no longer using ios-sim-xc
		if ios_sim_binary == "ios_sim.py":
			import ios_sim
			try:
				...
				LOG.info("Using device: %s" % devicetype_id)

				devicesdk = build.tool_config.get('ios.devicesdk') or None
				if devicesdk:
					LOG.info("Using devicesdk: %s" % devicesdk)

				device = ios_sim.device(devicetype_id, devicesdk)
				LOG.info("Found device: {devicetype} {runtime} {id}".format(
				...
				LOG.info("Installing app on simulator: %s" % path_to_app)
				ios_sim.install(device, path_to_app)

				package_name = _generate_package_name(build)
				LOG.info("Starting app: %s" % package_name)
				ios_sim.launch(device, package_name)

				logfile = ios_sim.log(device)
				LOG.info("Showing log output: %s" % ios_sim.log(device))
		...
		try:
			...
			if devicetypeid is not None:
				ios_sim_cmd = ios_sim_cmd + ['--devicetypeid', devicetypeid]
			else:
				ios_sim_cmd = ios_sim_cmd + ['--devicetypeid', 'com.apple.CoreSimulator.SimDeviceType.iPhone-6, 8.1']
	...
	def run_iphone_device(self, build, device, provisioning_profile, certificate=None, certificate_path=None, certificate_password=None):
		...
		if sys.platform.startswith('darwin'):
			...
			ios_deploy = [ensure_lib_available(build, 'ios-deploy'), '-d', '-u', '-t', '60', '-I', '-b', path_to_app]

...
def create_apk(build, output_filename, interactive=True):
...
def run_android(build, build_type_dir, sdk, device, interactive=True,
	...
	with temp_file('.apk') as out_apk_name:
			...
			with zipfile.ZipFile(zipf_name, 'r') as zipf:
				with zipfile.ZipFile(compressed_zipf_name, 'w') as compressed_zipf:
					for name in zipf.namelist():
						...
						elif target == 'crosswalk' and name == 'resources.arsc':
							compress_type = zipfile.ZIP_DEFLATED
						elif target == 'crosswalk' and name.startswith('lib/'):
							compress_type = zipfile.ZIP_DEFLATED
						elif target == 'crosswalk' and name.startswith('res/'):
							compress_type = zipfile.ZIP_DEFLATED
